...
class <?= ucfirst($table_name); ?> extends API
{
	...
	public function all_get()
	{
		...
		${table_name}s = array_map(function($row){
			<?php foreach ($field_relations as $field => $relation): 
	                ?>$row-><?= $field ?> = $this->db
			    ->get_where('<?= $relation['relation_table'] ?>', [
			    	'<?= $relation['relation_value'] ?>' => $row-><?= $field ?>])
			    ->row();
	        <?php endforeach; ?>
	...
	public function detail_get()
	{
		...
		$select_field = ['<?= implode("', '", $show_in_column); ?>'];
		${table_name} = $this->model_api_{table_name}->find($id, $select_field);
		...
		<?php foreach ($field_relations as $field => $relation): 
                ?>${table_name}-><?= $field ?> = $this->db
		    ->get_where('<?= $relation['relation_table'] ?>', [
		    	'<?= $relation['relation_value'] ?>' => ${table_name}-><?= $field ?>])
		    ->row();
        <?php endforeach; ?>
