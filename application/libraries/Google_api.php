...
class Google_api
{
    ...
    public function __construct()
    {
        ...
        $this->client_id = get_option('drive_client_id');
        $this->client_secret = get_option('drivve_client_secret');

        $this->client = new \Google_Client();
        $this->client->setAccessType('offline');
        $this->client->setClientId(get_option('drive_client_id'));
        $this->client->setClientSecret(get_option('drive_client_secret'));
        $this->client->setScopes(['https://www.googleapis.com/auth/drive']);

        //$this->initDriveService();


        $token = $this->client->refreshToken(get_option('drive_refresh_token'));
        ...
        if ($access_token) {
            set_option('drive_token', $access_token);
        }

        $refresh_token = $this->client->getRefreshToken();
        if ($refresh_token) {
            set_option('drive_refresh_token', $refresh_token);
        }

        $this->setAccessToken($_SESSION['GOOGLE_ACCESS_TOKEN']);
