...
class Model_crud extends MY_Model
{
	...
	public function get_crud_action($id)
	{
		$result = $this->db
			...
			->get_where('crud_action', ['crud_id' => $id])->result();
	...
	public function get_crud_field_condition($id)
	{
		$conditions = [];

		$result = $this->db->get_where('crud_field_condition', ['crud_id' => $id])->result();

		foreach ($result as $row) {
			$conditions[$row->crud_field_id][] = $row;
		}
